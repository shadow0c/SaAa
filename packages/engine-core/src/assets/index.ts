// src/assets/index.ts
//
// Motor seviyesinde Asset API — glTF/GLB model ve doku import, "derleme"
// (compile) raporu. Editör VE oyun bu API üzerinden asset yükler; ikisi de
// aynı yükleme/derleme mantığını kullanır (kod tekrarı yok).
...
function analyzeMaterialsAndTextures(root: THREE.Object3D): { materials: number; textureBytes: number } {
...
/**
 * Motor seviyesinde Asset API — TEK bir yükleme/derleme yolu.
 * Editör: import edip sahneye yerleştirmek için kullanır.
 * Oyun: aynı sınıfı runtime'da harita/prop yüklemek için kullanabilir.
 */
export class AssetAPI {
  ...
  /** ArrayBuffer'dan glTF/GLB parse eder + "derleme" raporu çıkarır. */
  async importModelBuffer(name: string, buffer: ArrayBuffer): Promise<AssetRecord> {
    ...
    const { materials, textureBytes } = analyzeMaterialsAndTextures(root);
  ...
  async importModelFile(file: File): Promise<AssetRecord> {
    ...
    return this.importModelBuffer(file.name, buffer);
  }
  ...
  /** Sahneye eklemeye hazır BAĞIMSIZ bir kopya (orijinal kayıt asla sahneye eklenmez). */
